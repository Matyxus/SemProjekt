...
from Components import Junction, Edge, Route, Struct
from constants import CWD, file_exists, JUNCTION_START_COLOR, JUNCTION_START_END_COLOR, JUNCTION_END_COLOR
from Utils.RouteManager import RouteManager


class Loader:
    """ Loads graph from .net.xml file """

    def __init__(self, file_name: str, struct: Struct, route_manager: RouteManager):
        self.cwd: str = str(CWD)
        print(f"Creating Loader Class, loading file form: {self.cwd}\\maps\\sumo\\{file_name + '.net.xml'}")
        assert(file_exists(f"{self.cwd}\\maps\\sumo\\{file_name+'.net.xml'}"))
        # Xml root of .net.xml file
        self.root: ET.Element = ET.parse(f"{self.cwd}\\maps\\sumo\\{file_name+'.net.xml'}").getroot()
    ...
    def load_junctions(self) -> None:
        ...
        saves them as {junction_id : Junction Class} in dictionary
        ...
        for junction in self.root.findall("junction"):
            ...
            if ("type" in junction.attrib) and (junction.attrib["type"] != "internal"):
                self.struct.junctions[junction.attrib["id"]] = Junction(junction.attrib)
        # print(end="\n")
    ...
    def load_edges(self) -> None:
        ...
        for edge in self.root.findall("edge"):
            ...
            if not ("function" in edge.attrib):
                ...
                self.struct.edges[edge_id] = Edge(edge.attrib)
                # Give each edge its lanes
                for lane in edge.findall("lane"):
                    self.struct.edges[edge_id].add_lane(lane.attrib)
                # Create route from edge
                route_id: int = self.get_route_id(edge_id)
                # Set destination junction in_route as this one
                self.struct.junctions[edge.attrib["to"]].neighbours[route_id] = []
        # -------------------  Assign routes, to junctions -------------------
        for route in self.struct.routes.values():
            # Routes only have 1 edge each
            edge_id: str = route.edge_list[0]
            from_junction: Junction = self.struct.junctions[self.route_manager.get_start(route.id)]
            if edge_id in connections:
                ...
                self.struct.starting_junctions.add(from_junction.attributes["id"])
                # print(f"Junction: {from_junction.attributes['id']} is starting !")
        # ------------------- Check -------------------
        for edge in self.struct.edges.values():
            # print(f"Checking edge: {edge.attributes['id']}")
            route: Route = self.struct.routes[self.edge_to_route[edge.attributes["id"]]]
            from_junction: Junction = self.struct.junctions[edge.attributes["from"]]
            to_junction: Junction = self.struct.junctions[edge.attributes["to"]]
        ...
        for junction_id, junction in self.struct.junctions.items():
            ...
            if (len(in_routes) == 1) and (len(out_routes) == 1):
                ...
                if self.route_manager.get_destination(out_route_id) == self.route_manager.get_start(in_route_id):
                    self.struct.starting_junctions.add(junction_id)
                    self.struct.ending_junctions.add(junction_id)
                    junction: Junction = self.struct.junctions[junction_id]
                ...
                self.struct.ending_junctions.add(junction_id)
                self.struct.junctions[junction_id].set_color(JUNCTION_END_COLOR)
                # print(f"Junction: {junction_id} is ending!")
                # print(junction_id, self.struct.junctions[junction_id].neighbours)
    ...
    def get_route_id(self, edge_id: str) -> int:
        ...
        assert (route_id not in self.struct.routes)
        self.struct.routes[route_id] = Route(route_id, [edge_id])
