...
import Utils.Loader
from Components import Struct, Route, deepcopy
from constants import CWD, file_exists, JUNCTION_DEFAULT_COLOR
from Utils.RouteManager import RouteManager
from matplotlib import pyplot as plt


class Graph(Struct):
    """ Graph made from .net.xml file """
    ...
    # Imported methods
    from MyGraph._simplify_graph import simplify_roundabouts, simplify_junctions, junction_can_be_removed
    from MyGraph._plot_graph import plot, default_plot, add_label, show_plot, make_legend, plot_default_graph
    from MyGraph._path_finder import reconstruct_path, dijkstra, A_star, TopK_A_star
    ...
    def create_sub_graph(self, start_junction_id: str, target_junction_id: str, c: float, plot: bool):
        ...
        """
        print(f"Creating sub-graph, from: {start_junction_id} to {target_junction_id}, c: {c}")
        # -------------------------------- checks --------------------------------
        ...
        other_routes: List[Route] = self.TopK_A_star(start_junction_id, target_junction_id, c, plot)
        if len(other_routes) == 0:  # Shortest path between start and destination does not exist
            return None
    ...
    def load_from_file(self, map_name: str) -> bool:
        ...
        if not file_exists(f"{str(CWD)}\\maps\\sumo\\{map_name+'.net.xml'}"):
            print(f"File {str(CWD)}\\maps\\sumo\\{map_name+'.net.xml'} does not exist!")
            return False
        self.map_name = map_name
        loader: Utils.Loader.Loader = Utils.Loader.Loader(map_name, self.get_skeleton(), self.route_manager)
